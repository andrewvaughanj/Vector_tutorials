...
void Manager::AddIncludedDessert(OrderType *Order) {
  ...
  if (Order->Entree == Steak && Order->Salad == Caesar &&
      Order->Beverage == MixedDrink) {

    Order->Dessert = Pies;

  } else if (Order->Entree == Lobster && Order->Salad == Green &&
             Order->Beverage == Wine) {
...
void Manager::PlaceOrder(unsigned int Table, unsigned int Seat,
                         OrderType Order) {
...
void Manager::ClearTable(unsigned int Table) { Data.DeleteRecord(Table); }
